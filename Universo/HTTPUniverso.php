...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    //VALIDA LA BUSQUEDA SEGUN EL ID DEL UNIVERSO
    if (isset($_GET['ID_UNIVERSO'])) {
        ...
        if (isset($_GET['NOMBRE_JUEGO'])) {
            $sql = $dbConn->prepare("SELECT * FROM `universomagico` LEFT JOIN juego on juego.ID_UNIVERSO=universomagico.ID_UNIVERSO WHERE NOMBRE_JUEGO LIKE '%' :NOMBRE_JUEGO '%'");
            ...
            if ($row_count == 0) {
                ...
                echo "Si existe el registro  ";
                $sql = $dbConn->prepare("SELECT juego.NOMBRE_JUEGO, juego.DESCRIPCION_JUEGO, juego.INSTRUMENTO_JUEGO, universomagico.TEMA_UNIVERSO FROM `universomagico` LEFT JOIN juego on juego.ID_UNIVERSO=universomagico.ID_UNIVERSO WHERE NOMBRE_JUEGO LIKE '%' :NOMBRE_JUEGO '%'");
                $sql->bindValue(':NOMBRE_JUEGO', $_GET['NOMBRE_JUEGO']);
                $sql->execute();
                header("HTTP/1.1 200 OK");
                echo json_encode($sql->fetchAll(PDO::FETCH_ASSOC));
            ...
            if (isset($_GET['NOMBRE_MONEDA'])) {
                $sql = $dbConn->prepare("SELECT * FROM `universomagico`LEFT JOIN moneda on moneda.ID_UNIVERSO=universomagico.ID_UNIVERSO WHERE NOMBRE_MONEDA LIKE '%' :NOMBRE_MONEDA '%'");
                ...
                if ($row_count == 0) {
                    ...
                    //REALIZA LA BUSQUEDA Y OBTIENE LOS DATOS
                    echo "Si existe el registro  ";
                    $sql = $dbConn->prepare("SELECT moneda.NOMBRE_MONEDA, moneda.CAMBIO_MONEDA, moneda.MATERIAL_MONEDA, universomagico.TEMA_UNIVERSO, universomagico.AUTOR_UNIVERSO FROM `universomagico` LEFT JOIN moneda on moneda.ID_UNIVERSO=universomagico.ID_UNIVERSO WHERE NOMBRE_MONEDA LIKE '%' :NOMBRE_MONEDA '%'");
                    $sql->bindValue(':NOMBRE_MONEDA', $_GET['NOMBRE_MONEDA']);
                    $sql->execute();
                    header("HTTP/1.1 200 OK");
                    echo json_encode($sql->fetchAll(PDO::FETCH_ASSOC));
                ...
                if (isset($_GET['NOMBRE_TRANSPORTE'])) {
                    $sql = $dbConn->prepare("SELECT * FROM `universomagico` LEFT JOIN transporte on transporte.ID_UNIVERSO=universomagico.ID_UNIVERSO WHERE NOMBRE_TRANSPORTE LIKE '%' :NOMBRE_TRANSPORTE '%'");
                    ...
                    if (isset($_GET['EFECTO_HECHIZO'])) {
                        $sql = $dbConn->prepare("SELECT * FROM `universomagico` LEFT JOIN hechizo on hechizo.ID_UNIVERSO=universomagico.ID_UNIVERSO WHERE EFECTO_HECHIZO LIKE '%' :EFECTO_HECHIZO '%'");
                        ...
                        if ($row_count == 0) {
                            ...
                            //REALIZA LA BUSQUEDA Y OBTIENE LOS DATOS
                            echo "Si existe el registro  ";
                            $sql = $dbConn->prepare("SELECT hechizo.NOMBRE_HECHIZO, hechizo.EFECTO_HECHIZO, hechizo.DURACION_HECHIZO, universomagico.TEMA_UNIVERSO, universomagico.AUTOR_UNIVERSO FROM `universomagico` LEFT JOIN hechizo on hechizo.ID_UNIVERSO=universomagico.ID_UNIVERSO WHERE EFECTO_HECHIZO LIKE '%' :EFECTO_HECHIZO '%'");
                            $sql->bindValue(':EFECTO_HECHIZO', $_GET['EFECTO_HECHIZO']);
                            $sql->execute();
                            header("HTTP/1.1 200 OK");
                            echo json_encode($sql->fetchAll(PDO::FETCH_ASSOC));
